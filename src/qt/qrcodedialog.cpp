...
{
    QString ret = QString("pocketchange:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non PKT unit input in lnReqAmount, we generate the URI with BTC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(PocketChangeUnits::format(PocketChangeUnits::PKT, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
