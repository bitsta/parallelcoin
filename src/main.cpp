...
const string strMessageMagic = "PocketChange Signed Message:\n";
...
/** Amount of pocketchanges spent by the transaction.
...
static const int64 nMinSubsidy = 22 * COIN;
...
void ThreadScriptCheck() {
    RenameThread("pocketchange-scriptch");
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PocketChangeMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PocketChangeMiner : ProcessBlock, block not accepted");
    }
...
void static PocketChangeMiner(CWallet *pwallet)
{
...
void static ThreadPocketChangeMiner(CWallet *pwallet)
{
    printf("PocketChange miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pocketchange-miner");
    ...
    {
        printf("PocketChange miner terminated\n");
        throw;
    }
}

void GeneratePocketChanges(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ThreadPocketChangeMiner, pwallet));
}
